...
const PaymentFail = () => {
    ...
    const [message, setMessage] = useState('Payment failed or was cancelled.');
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const updateOrderStatusOnFail = async () => {
            try {
                ...
                if (responseData.success && responseData.data.orderStatus !== 'completed') {
                    setMessage(`Payment failed for Order ID: ${orderId}. Please try again.`);
                    toast.error(`Payment failed for order ${orderId}.`);
                } else if (responseData.success && responseData.data.orderStatus === 'completed') {
                    setMessage(`Payment was successful for Order ID: ${orderId} despite being redirected to fail page. Please check your orders.`);
                    toast.info(`Payment was successful for order ${orderId}.`);
                } else {
                    setMessage(`Payment failed for Order ID: ${orderId}. ${responseData.message || ''}`);
                    toast.error(`Payment failed for order ${orderId}.`);
                }
            } catch (error) {
                console.error("Error fetching order status on fail page:", error);
                setMessage('An error occurred. Payment might have failed.');
                toast.error('An error occurred during payment failure processing.');
            } finally {
                setLoading(false);
            }
        };
